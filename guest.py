...
def show_bills():
    ...
    '''
    g_id = current_user.guest_id
    
    incurs_bills = IncursBill.query.filter(IncursBill.guest_id == g_id).all()
    bill_id = incurs_bills.bill_id
    all_paid_bills = Bill.query.filter_by(bill_id=bill_id, paid_status='0').all()
    ...
    if all_unpaid_bills:
        total_amount = sum(bill_entry.amount for bill_entry in all_unpaid_bills)
